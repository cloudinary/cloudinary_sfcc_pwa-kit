...
import RenderContentAssets from '../../components/cloudinary-content-asset'
import RenderContentSlots from '../../components/cloudinary-content-slots'
import { config } from '../../config'
import { contentAssetParams } from '../../constants'
 ...
 */
const Home = ({ productSearchResult, isLoading, contentResult, slotResult }) => {
    const intl = useIntl()
    return (
        <Box data-testid="home-page" layerStyle="page">
            ...
            <Hero
                title={intl.formatMessage({
                    ...
                    id: 'New Slot Configuration - 2022-08-18 15:49:38'
                })}
                img={{
...
Home.getProps = async () => {
    ...
    `https://${config.parameters.host}/on/demandware.store/Sites-${config.parameters.siteId}-Site/default/CloudinaryStaticContent-RenderAsset?contentId=${contentAssetParams.CONTENT_ASSET_ID}`
    )
    if (res.ok) {
        contentResult = await res.text()
    }
    let slotResult;
    //Make a call to get Content Slots from SFCC
    const slotRes = await fetch(
        `https://${config.parameters.host}/on/demandware.store/Sites-${config.parameters.siteId}-Site/default/CloudinaryStaticContent-RenderSlots`
    )
    if (slotRes.ok) {
        slotResult = await slotRes.text()
    }
    if (process.env.NODE_ENV !== 'production') {
        console.log(contentResult)
        console.log(slotResult)
    }
    return { contentResult: contentResult, slotResult: slotResult }
}
